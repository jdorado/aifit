...
const WorkoutOverflowMenu: FC<WorkoutOverflowMenuProps> = ({
  ...
  const handleClear = useCallback(() => {
    if (!canEditPlan || !canClearWorkout) return
    closeAfter(onClearWorkout)
  }, [canClearWorkout, canEditPlan, closeAfter, onClearWorkout])

  const clearLabel = clearMode === 'unlogged'
    ? t('workout.clearUnloggedAction')
    : t('workout.clearAction')
  ...
  return (
    <div className="header-menu" ref={menuRef}>
      <button
        ...
        aria-label={copyingLastWeek ? t('workout.copyLastWeekLoading') : t('common.more')}
        title={copyingLastWeek ? t('workout.copyLastWeekLoading') : t('common.more')}
        aria-busy={copyingLastWeek}
        aria-expanded={open}
        aria-haspopup="menu"
        disabled={copyingLastWeek}
      >
        {copyingLastWeek ? <span className="spinner" aria-hidden="true" /> : (
      ...
      {open ? (
        <div className="header-menu-panel" role="menu">
          ...
          <button
            ...
            type="button"
            role="menuitem"
            onClick={() => closeAfter(onGenerateWithCoach)}
          ...
          <button
            ...
            onClick={() => closeAfter(onCopyLastWeek)}
          >
            <svg className="header-menu-item-icon" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <button
            ...
            role="menuitem"
            onClick={handleClear}
            disabled={!canEditPlan || !canClearWorkout}
          >
            <svg className="header-menu-item-icon" viewBox="0 0 24 24" aria-hidden="true">
              <path
