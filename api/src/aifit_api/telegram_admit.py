...
this bridge their runs execute with no plugin context and every AIFit plugin
command refuses with "no scoped application context".
...
effect for subsequent commands without restarting or duplicating the turn.
...
ADMIT_PERMISSIONS = frozenset({"aifit:read", "aifit:write"})


def admit_enabled() -> bool:
    """Enabled by default; the flag is a kill switch, not an opt-in."""
    return os.getenv("AIFIT_TELEGRAM_ADMIT", "1").strip() != "0"


def poll_seconds(default: int = 5) -> int:
    try:
        return max(2, int(os.getenv("AIFIT_TELEGRAM_ADMIT_POLL_SECONDS", str(default))))
    except ValueError:
        return default
...
def _is_telegram_intake(entry: object) -> bool:
    """Only the relay's unadmitted Telegram projection carries telegramUserId."""
    if not isinstance(entry, dict):
        return False
    return (
        entry.get("scope") == "telegram"
        and entry.get("telegramUserId") is not None
...
async def poll_once(
    ...
    for binding in bindings:
        ...
        for entry in entries:
            if not _is_telegram_intake(entry):
                continue
            run_id = entry["id"]
            stats["runs_seen"] += 1
