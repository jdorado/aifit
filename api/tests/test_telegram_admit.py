...
def test_poll_skips_non_telegram_entries_and_unknown_accounts(tmp_path):
    ...
    async def handler(request):
        if request.method == "GET" and request.url.path == "/v1/runs":
            return httpx.Response(200, json={"runs": [
                ...
                {"id": "tg_done", "scope": "telegram", "status": "completed", "telegramUserId": 42},
                {"id": "tg_1", "scope": "telegram", "status": "running", "telegramUserId": 42},
            ]})
        return httpx.Response(404, json={"error": "nope"})

    async def attach_check():
        async with _client(handler) as client:
            return await telegram_admit.poll_once(
                client, bindings, _find_account, _build_context, set())

    # Completed intake is never attached; acc_missing has no account, so the
    # running tg_1 is seen twice and attaches zero times.
...
def test_poll_treats_conflict_as_already_admitted_and_tolerates_errors(tmp_path):
    ...
    async def conflict(request):
        if request.url.path == "/v1/runs":
            return httpx.Response(200, json={"runs": [
                {"id": "tg_7", "scope": "telegram", "status": "running", "telegramUserId": 42},
            ]})
...
def test_admit_is_enabled_by_default_with_a_kill_switch(monkeypatch):
    monkeypatch.delenv("AIFIT_TELEGRAM_ADMIT", raising=False)
    assert telegram_admit.admit_enabled() is True
    monkeypatch.setenv("AIFIT_TELEGRAM_ADMIT", "0")
